 ...
 ******************************************************************************
 * @file           : main.c
 * @brief          : Main program file for Faraday BMS
 ******************************************************************************
 * @attention
 *
 * All rights reserved.
...
static uint8_t UART_PrepModeStatusMessage3();
...
/*
 * Prep mode selector 3 message (8 bytes). Refer to documentation for packet format
 * Respond to battery status message 1 - battery OK.
 *
 * Hard coded for now
 * Example request [0x3, 0x3, 0x0, 0x0, 0x0, 0x3, 0x4, 0x29]
 * Example response  0x3 0x3 0x6 0x0 0x0 0x0 0x2 0x3f 0xff 0xc8 0x65 //0x2 is off, 0x0 is low, 0x1 is fast?
 * Example response  0x3 0x3 0x6 0x0 0x0 0x0 0x0 0x3f 0xff 0x69 0xa5
 * Example response  0x3 0x3 0x6 0x0 0x0 0x0 0x1 0x3f 0xff 0x38 0x65
 */
uint8_t UART_PrepModeStatusMessage3() {
	...
	//add crc in reverse byte order
	uint16_t crc = UART_CRC(UART_TxData, 9);
	UART_TxData[9] = crc & 0xFF;
	UART_TxData[10] = crc >> 8;
	UART_TxData[11] = 0xFF; //add extra byte of zeros to match faraday protocol
	return 11;
}
	...
	if (size == 8) {
		...
		if (CRCRecv == UART_CRC(buf, 6) && SlaveID == 0x02) {
			...
			if (FunctionCode == 0x03) { //it's a read
				...
				/* added for bikes without mode selector AT ALL */

			}
		/* Added for no mode selector bikes  */
		} else if (SlaveID == 0x03){
			if (FunctionCode == 0x03) { //it's a read
					if (Address == 0x00) { //Mode Selector Request 1
						len = UART_PrepModeStatusMessage3();
						THVD2410_Transmit();
						HAL_UART_Transmit(&huart2, UART_TxData, len,
						UART_TIMEOUT_S);
