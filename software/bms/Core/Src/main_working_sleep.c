...
void UART_PrepCellVoltageMessage();
void UART_PrepCellBalancingMessage();
...
// See the TRM or the BQ76952 header file for a full list of Subcommands
{
	...
	if (type == R) {	//read
		...
		I2C_ReadReg(0x40, RX_32Byte, 16); //more then 16 would cause CRC errors, so I limit this to 16. This does mean that large data reads are not supported.
	} else if (type == W) {
...
void BQ769x2_Init() {
	...
	BQ769x2_SetRegister(BalancingConfiguration, 0x03, 1); //Set balancing to autonomously operate while in RELAX and CHARGE configurations
	BQ769x2_SetRegister(CellBalanceMaxCells, 0x03, 1); //0x933A  - set maximum number of cells that may balance at once. Contributes to thermal limit
	BQ769x2_SetRegister(CellBalanceMinCellVCharge, 0x0E74, 2); //0x933B -Minimum voltage at which cells start balancing. Set to 3700mV for now

	/*Over and Under Voltage configuration*/
	BQ769x2_SetRegister(CUVThreshold, 0x31, 1); //CUV (under-voltage) Threshold - 0x9275 = 0x31 (2479 mV) this value multiplied by 50.6mV
	BQ769x2_SetRegister(COVThreshold, 0x53, 1); //COV (over-voltage) Threshold - 0x9278 = 0x55 (4199 mV) this value multiplied by 50.6mV
	...
	BQ769x2_SetRegister(DsgCurrentThreshold, 0x64, 2); //0x9310   Set definition of discharge in mA
	BQ769x2_SetRegister(ChgCurrentThreshold, 0x32, 2); //0x9312  Set definition of charge in mA
...
;
//double check that this was successful
...
uint8_t BQ769x2_Reset() {
	...
	while (retry_counter < 20) {
		...
		delayUS(60000);
		delayUS(60000);
		delayUS(60000); //takes a while for everything to stabilize
		BQ769x2_ReadBatteryStatus();
 ...
 */
void UART_PrepBatteryStatusMessage1() {
 ...
 */
void UART_PrepBatteryStatusMessage2() {
		...
		if (CRCRecv == UART_CRC(buf, 6) && SlaveID == 0x02) {
			if (FunctionCode == 0x03) { //it's a read
				...
				if (Address == 0x02) { //Battery Voltage Message
					UART_PrepCellVoltageMessage();
					THVD2410_Transmit();
					HAL_UART_Transmit(&huart2, TxData, 29, 3);

				} else if (Address == 0x17) { //Battery Balancing Message
					UART_PrepCellBalancingMessage();
					THVD2410_Transmit();
					HAL_UART_Transmit(&huart2, TxData, 7, 3);

				} else if (Address == 0x00 && NumRegs == 0x01) { //Battery Status Message 1
					UART_PrepBatteryStatusMessage1();
					THVD2410_Transmit();
					HAL_UART_Transmit(&huart2, TxData, 7, 3);
...
int main(void) {
	...
	HAL_Delay(30); //Wait for everything to stabilize
	...
	while (1) {
		...
		while (1) {
			...
			if (AlarmBits & 0x80) { // Check if FULLSCAN is complete. If set, new measurements are available
				...
				BQ769x2_ReadAllVoltages(); //needed for sending voltages to mode selector
			...
			delayUS(50000);  // repeat loop every 20 ms
 ...
 * @retval None
 */
void SystemClock_Config_4MHz(void) {
	RCC_OscInitTypeDef RCC_OscInitStruct = { 0 };
	RCC_ClkInitTypeDef RCC_ClkInitStruct = { 0 };
	RCC_PeriphCLKInitTypeDef PeriphClkInit = { 0 };
...
//htim2.Init.Prescaler = 31; for 32Mhz
	htim2.Init.Prescaler = 16; //for 4MHz
...
void Error_Handler(void) {
	...
	if (DEBUG) {
		...
		HAL_Delay(1000); //wait a second and let's try this out again
		NVIC_SystemReset();
	}
