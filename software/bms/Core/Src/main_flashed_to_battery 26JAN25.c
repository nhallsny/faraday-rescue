...
uint8_t BUTTON_PRESSED_DURING_BOOT = 0;
...
/*Double Check that the device has been configured. If not, configure it.*/
uint8_t BQ769x2_IsConfigured() {

	BQ769x2_Init();

	//Verify that device is not in config update mode
	BQ769x2_ReadBatteryStatus();
...
/*Put the BQ into DEEPSLEEP by writing the DEEPSLEEP command twice. Returns 0 if successful, otherwise 1*/
...
/* BQ769x2_Reset - hard reset of the BQ chip. Returns 0 if successful, otherwise returns the number of retry attempts */
uint8_t BQ769x2_Reset() {
	...
	while (retry_counter < 20) {
		...
		BQ769x2_CommandSubcommand(BQ769x2_RESET); // Resets the BQ769x2 registers
...
uint8_t BQ769x2_ReadSafetyStatus() { //good example functions
...
//prep the cell voltage message (28 bytes). Refer to documentation for packet format.
uint8_t UART_PrepCellVoltageMessage() {
	...
	return 30; //add two bytes of zeros to meet faraday protocol
}

//prep the cell balancing message (7 bytes). Refer to documentation for packet format
uint8_t UART_PrepCellBalancingMessage() {
	...
	UART_TxData[6] = crc >> 8;
	UART_TxData[7] = 0xFF;
	return 8; //add two bytes of zeros to meet faraday protocol
}

/*
 * Prep battery status 1 message (7 bytes). Refer to documentation for packet format
...
void STM32_HandleButton() {
	...
	if (BUTTON_COUNT > BUTTON_LONG_PRESS_LOOPS) {
		if (DEBUG) {
			printf("\r\ntime to sleep\r\n");
		}
		BUTTON_COUNT = 0;
...
//Increment sleep timer if current is between +20mA and -250mA
	...
	if (!DEBUG) {
		if (INACTIVITY_COUNT > INACTIVITY_LOOPS_MAX) {
			Sleep();
		}
...
/* Check to see if the magic value was written into the persistent RTC registers on the last reboot */
...
int main(void) {
	...
	BQ769x2_ReleaseShutdownPin(); // RST_SHUT pin set low just in case
	...
	while (1) {
		...
		*/
		...
		if (!BUTTON_PRESSED_DURING_BOOT && Pack_Voltage < 15000) {
			Sleep();
		}
